...
start = content.find("pub(crate) fn recover_detached_sediment(")
end = content.find("fn validate_checkpoint_state(", start)
if start < 0 or end < 0:
    raise SystemExit("sediment recovery function boundary missing")
replacement = r'''pub(crate) fn recover_detached_sediment(
...
old_validate_start = content.find("fn validate_checkpoint_state(")
old_validate_end = content.find("#[cfg(test)]", old_validate_start)
if old_validate_start < 0 or old_validate_end < 0:
    raise SystemExit("obsolete checkpoint validation boundary missing")
content = content[:old_validate_start] + content[old_validate_end:]
recovery.write_text(content)

replace_once(
    "src/sand/mod.rs",
    "pub(crate) use recovery::{RecoveryTiming, recover_detached_sediment};",
    "pub(crate) use recovery::{\n    RecoveryTiming, recover_detached_sediment, settle_transition_sediment,\n};",
)
...
helper_start = content.find("fn settle_transition_sediment_segment(")
helper_end = content.find("fn valid_category_ids_for_catalog(", helper_start)
if helper_start < 0 or helper_end < 0:
    raise SystemExit("transition settlement helper boundary missing")
helper = content[helper_start:helper_end]
if "recover_detached_sediment(" not in helper:
    raise SystemExit("transition helper detached-recovery call missing")
helper = helper.replace("recover_detached_sediment(", "settle_transition_sediment(", 1)
...
test = r'''    #[test]
    fn transition_settlement_preserves_mass_on_uninitialized_canvas() {
        ...
        assert!(
            recover_detached_sediment(&state, &categories(), CategoryId::new(1), timing)
                .is_err()
        );
        let settled =
            settle_transition_sediment(&state, &categories(), CategoryId::new(1), timing)
                .unwrap();
...
recovery.write_text(content.replace(marker, test + marker, 1))
