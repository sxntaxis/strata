...
# Add the persistence-recovery state machine and wire App ownership.
...
app = replace_once(
    app,
    "mod view_style;\n",
    "mod view_style;\n\nuse persistence_recovery::{\n    PersistenceOperation, PersistenceRecoveryState, RecoveryAction,\n};\n",
    "persistence recovery imports",
)
app = replace_once(
    app,
...
app = replace_once(
    ...
    "startup persistence failure handling",
)

# Give transition failures the correct reconciliation policy.
app = replace_once(
    app,
...
impl fmt::Display for PersistenceOperation {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            ...
            Self::EmergencyExport => "emergency recovery export",
        })
    }
}
...
impl App {
    ...
    fn retry_persistence_failure(&mut self) {
        ...
        let result = match action {
            ...
            RecoveryAction::CommitCheckpointRecovery => {
                self.try_commit_checkpoint_recovery()
            }
        };
        ...
        match result {
            Ok(()) => {
                ...
                self.render_needed = true;
                if matches!(action, RecoveryAction::FinishAndExit | RecoveryAction::DetachAndExit) {
                    self.recovery_exit_requested = true;
    ...
    fn export_emergency_recovery(&self) -> Result<PathBuf, String> {
        ...
        let sessions = self
            ...
            .collect();
        let active = self.session.active_session_started_at_utc.map(|started_at| {
            EmergencyActiveSession {
...
mod tests {
    ...
    fn persistence_failure_classes_are_actionable() {
        assert_eq!(classify_failure("database is locked"), PersistenceFailureClass::Busy);
        assert_eq!(
