...
updated: 2026-09-05
authority: working
summary: H4/v5 remains published production authority and SEDIMENT-008 oslo-vessel-front physics remains owner-frozen. SEDIMENT-015A-R2 through 015E are semantically machine-green. 015F direct per-grain geometry improved continuity but the owner still rejected large hollow diagonal channels inside the avalanche. Reinspection corrected the renderer diagnosis: 015F already replays captured targets; the remaining defect is transient lane assignment against hidden physical support and pre-final rolling occupancy. SEDIMENT-015G owns batch-final visual-support geometry while preserving 015F as a control. Its first native run stopped on an invalid synthetic two-edge fixture before testing the contiguous-lane semantic; R1 corrects only that test geometry and leaves runtime source byte-unchanged.
next: Natively validate SEDIMENT-015G-R1. Run the corrected contiguous-lane gate first, then prove rolling targets remain pending until the coherent physical quantum is complete, final lanes are contiguous above the delayed visual shadow rather than hidden physical settlement, finalized geometry is immutable during replay, 015F and all earlier foundations stay green, frozen-front/exact-stack parity remains exact, and truthful raster performance does not regress before another owner rainbow-collapse gate.
---
