...
updated: 2026-08-30
authority: working
summary: The clean H5+H9 candidate passed native composition proof, but owner visual acceptance rejected H5 because deliberate widening left former lateral walls frozen as interior cliffs. H9 remains native-certified and visually smoother, with synchronized cell-jump and fast-slip observations deferred. H10 boundary-release lineage + SandState v6 is now the authored repair candidate.
next: Native-certify H10 on top of the clean H5+H9 composition. Prove persistent left/right/incremental resize release, exact v6 restart including an in-flight release grain, v5→v6 compatibility, unchanged H4/H9 semantics, and full regressions. Do not merge the rejected H5+H9 publication candidate. Repeat only a short resize human pass after H10; general avalanche physics, synchronized falling-dot jump, slip-friction cadence, and ingress-focus padding remain separate later units.
---
...
PLATEAU-001H / H5 — one-shot resize boundary release — is **NATIVE-CERTIFIED IMPLEMENTATION EVIDENCE / OWNER-REJECTED AS SUFFICIENT PRODUCT BEHAVIOR** at `3920ab3899f3249569f2dfb8c990e6389cb6fc47` / tree `8f3f69f7baeb15cd60423e5663b36647efd4a68b`, based on current main `b5afc619bace6d9d98ec14ccd065add3689e2e38`. Focused H5 tests, 22 focused H4/organic tests plus one ignored behavior bench, 272 full library tests plus one ignored bench, 23 integration tests, formatting, strict Clippy, help smoke, and diff hygiene all passed; the only fallout was rustfmt-only. A later clean H5+H9 composition also passed native equivalence at `58b6a5d91b2fae2bdb4b244e793416b8fffab8c8`, but deliberate owner widening visibly left former side walls as frozen interior cliffs. The clean publication candidate must therefore not merge.
...
The clean H5+H9 publication composition was natively certified with exact H5 engine and H9 app blobs and no H6/H8 experiment machinery. Owner visual use then supplied three distinct observations: ordinary falling motion is markedly smoother under H9; all falling dots appear to share a periodic synchronized spatial jump; and diagonal slips feel uniformly faster or less frictional than expected. General avalanche visibility remained inconclusive because the pile was still relatively low and the deliberate resize test introduced the H5 frozen-wall defect. These presentation observations are retained but do not authorize cadence/friction changes inside H10.

PLATEAU-001H / H10 — persistent resize boundary-release lineage + SandState v6 — is **AUTHORED / NATIVE CERTIFICATION PENDING**. H10 replaces H5's one-shot mobilization only at the resize boundary. A former wall with outward relief `>1` creates a release domain with fixed direction, former-wall coordinate, and deepest admitted inward coordinate. Normal gravity may perform at most one exact boundary-release diagonal topple; the already-admitted face is rechecked from the wall inward before another inward column may be admitted, so deeper material cannot refill an outward column and leave a replacement cliff. Distinct same-side release events may coexist while disjoint and normalize only when their causal domains meet. A boundary-released grain does not inherit H4 mobility from wall removal. Relief greater than two creates at most one separately tracked in-flight release grain; ordinary vertical gravity must reconnect it before boundary release advances again. Hidden domains/in-flight custody freeze with the hidden basin. Resize itself remains movement-free, RNG-free, and mass-neutral. SandState v6 persists release domains plus the optional in-flight coordinate; v5→v6 preserves exact H4 mobility and invents no release state. H9 scheduler source is frozen except for v6 construction compatibility.
...
Native-certify H10 from the clean H5+H9 base. Require left/right staircase and tall-cliff convergence, incremental/re-expansion behavior, canonical-growth coordinate shifts, exact v6 snapshot/restart including an in-flight release grain, v5→v6 recovery, malformed-state fail-closed checks, unchanged H4/H9 behavior, full application checks, and isolated runtime smoke. The local agent may make only mechanical behavior-preserving fallout fixes. Do not merge to main. After native green, repeat one short owner resize test before revisiting avalanche visibility, synchronized falling-dot jumps, slip-friction cadence, or ingress-focus padding.
