...
updated: 2026-08-30
authority: working
summary: H5 resize boundary release is native-certified; H6 threshold-cross propagation is rejected; H7 confirms latent supported rupture opportunity; PLATEAU-001H H8 now benches an exact-path relief-2 rupture token without changing production semantics.
next: Execute the H8 exact-path rupture-token A/B bench at 4,000 and 10,000 ingress, compare avalanche width and topology against unchanged H4, and reject it unless widening is material without degrading the H4 hill equilibrium. Keep ingress-focus padding later.
---
...
PLATEAU-001H / H8 — exact-path relief-2 rupture token — is **AUTHORED TEST-ONLY A/B / NATIVE EXECUTION REQUIRED**. Only a supported immediately-uphill grain that already had relief exactly 2 before the causal H4 topple may inherit a transient one-hop token toward that source column. The token moves that grain diagonally once but deliberately does not copy its mobility bit or invoke H4 same-column slip-lineage; subsequent contact/support behavior is ordinary H4. A contiguous relief-2 staircase may pass the token column by column. Relief 3+, H6 threshold crossings, already-mobilized grains, scans/radii, persistence, ingress, and production behavior are untouched. The A/B must show materially broader multi-column episodes without H6-like roughness/variance damage before any promotion is considered. Ingress-focus padding remains later.
