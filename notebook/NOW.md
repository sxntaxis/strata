...
updated: 2026-09-05
authority: working
summary: H4/v5 remains published production authority and SEDIMENT-008 oslo-vessel-front physics remains owner-frozen. SEDIMENT-015A-R2 through 015E are semantically machine-green. 015F direct per-grain geometry improved continuity but the owner still rejected large hollow diagonal channels inside the avalanche. SEDIMENT-015G batch-final visual-support geometry now passes constructor, visible-support and contiguous-lane native gates. R1 exposed a real playback defect: after a lane was finalized, the inherited micro tracking blend rendered the carrier behind the finalized coherent interpolation, so a later shadow mutation could leave unsupported visual holes even though `active.target_y` itself remained immutable. R2 makes the 015G visual-support profile render each active segment exactly at its coherent interpolation while preserving 015F and earlier profiles as controls.
next: Natively validate SEDIMENT-015G-R2. Run the former immutable-playback failure first, then re-run all 015G support gates, prove 015F and earlier control profiles remain unchanged, frozen-front/exact-stack parity remains exact, and truthful raster scaling stays green before another owner rainbow-collapse gate.
---
