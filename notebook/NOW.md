...
updated: 2026-08-30
authority: working
summary: H5 resize boundary release is native-certified; H6 and H8 propagation experiments are rejected; H7 confirms latent supported rupture opportunity; PLATEAU-001H H9 now separates ordinary live sediment presentation from the 120 ms accelerated catch-up cadence without changing H4 physics.
next: Natively certify H9 live-cadence separation, including focused scheduler thresholds, exact H4/H5 regression, full checks, and an isolated runtime proof that ordinary live physics is no longer grouped behind catch-up cadence. Keep further avalanche physics and ingress-focus padding deferred until H9 is judged in daily use.
---
...
Natively certify PLATEAU-001H / H9 live sediment cadence separation. Prove the scheduler thresholds, unchanged H4/H5 sediment semantics, unchanged bounded catch-up and transition-edge authority, full validation, and one isolated native runtime showing ordinary live physics is no longer grouped behind the 120 ms catch-up cadence. Do not add another avalanche propagation rule during this gate. After H9 is native-green, use short daily-use observation to decide whether the remaining avalanche deficiency is physical or was primarily perceptual. Ingress-focus padding remains a separate later deposition change.
