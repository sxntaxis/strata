...
impl SqliteRepository {
    pub(super) fn import_legacy(
        ...
        for receipt in &plan.lifecycle_receipts {
            transaction.execute(
        ...
        let expected_lifecycle_count =
            i64::try_from(plan.lifecycle_receipts.len()).map_err(|_| {
...
fn parse_lifecycle_receipts(bytes: &[u8]) -> Result<Vec<LegacyLifecycleImport>, LegacyImportError> {
    ...
    ledger
        .validate()
        .map_err(|message| invalid("category_lifecycle_ledger.json", None, message))?;
    ledger
        .receipts
        .iter()
    ...
    fn strict_import_preserves_sources_and_verifies_every_state_family() {
        ...
        let lifecycle_ledger = LegacyCategoryLifecycleLedger {
            ...
            receipts: vec![LegacyLifecycleReceipt {
                ...
                references:
                    crate::legacy_category_lifecycle::LegacyCategoryReferenceCounts::default(),
                applied_at_utc: "2026-08-01T14:00:00Z".parse().unwrap(),
            }],
        };
        crate::storage::write_json_atomic(&fixture.paths.lifecycle_ledger_json, &lifecycle_ledger)
            .unwrap();
        ...
        assert_eq!(
            next, 4,
            "migration must preserve retired identity high-water mark"
        );
    }
