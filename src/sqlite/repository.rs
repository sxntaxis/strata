...
mod tests {
        ...
        fn write_sources(&self) {
            ...
            fs::write(
                ...
                "id,date,category_id,category_name,description,start_time,end_time,elapsed_seconds\n\
                 7,2026-08-01,1,Study,Read,10:00:00,11:00:00,3600\n\
                 8,2026-08-02,2,Leisure,Game,12:00:00,12:30:00,1800\n",
            )
            .unwrap();
            let sand = r#"{
                "version": 1,
            ...
            fs::write(
                ...
                format!(
                    r#"{{
                        "schema_version": 1,
                        "detached_at_utc": "2026-08-02T19:00:00Z",
                        "simulation_time_utc": "2026-08-02T19:00:00Z",
                        "spawn_accumulator_nanos": 0,
                        "physics_accumulator_nanos": 0,
                        "active_category_id": 1,
                        "active_description": "Continue reading",
                        "active_session_started_at_utc": "2026-08-02T18:00:00Z",
                        "sand_state": {sand},
                        "pending_mutations": []
                    }}"#
                ),
            ...
            fs::write(
                ...
                r#"{
                    "version": 1,
                    "tags_by_category": {"1": ["focus", "reading"]}
                }"#,
            )
            .unwrap();
            fs::write(self.paths.sand_history_dir.join("2026-08-01.json"), sand).unwrap();
        }
    }
    ...
    fn consistent_snapshot_matches_legacy_domain_visible_state() {
        ...
        let legacy_categories =
            storage::try_load_categories_from_csv(&fixture.paths.categories_csv).unwrap();
        let legacy_sessions = storage::try_load_sessions_from_csv(
            &fixture.paths.sessions_csv,
            &legacy_categories.categories,
        )
        .unwrap();
        let plan = LegacyImportPlan::from_paths(
            &fixture.paths,
            LegacyImportOptions {
        ...
        let snapshot = repository.read_consistent_snapshot().unwrap();

        let sqlite_categories: Vec<_> = snapshot
            .categories
            .iter()
            .filter(|category| category.id != 0)
            .collect();
        let legacy_non_idle: Vec<_> = legacy_categories
            .categories
            .iter()
            .filter(|category| category.id.0 != 0)
            .collect();
        assert_eq!(sqlite_categories.len(), legacy_non_idle.len());
        for (sqlite, legacy) in sqlite_categories.iter().zip(legacy_non_idle.iter()) {
            assert_eq!(sqlite.id, legacy.id.0 as i64);
            assert_eq!(sqlite.name, legacy.name);
            assert_eq!(sqlite.description, legacy.description);
            ...
            .zip(legacy_sessions.sessions.iter())
        {
            ...
            assert_eq!(sqlite.description, legacy.description);
            assert_eq!(sqlite.elapsed_seconds, legacy.elapsed_seconds as i64);
        }
        assert_eq!(snapshot.sessions[0].started_at_utc, "2026-08-01T16:00:00Z");
        assert_eq!(snapshot.sessions[0].ended_at_utc, "2026-08-01T17:00:00Z");
        assert_eq!(
            snapshot.category_tags.get(&1),
            Some(&vec!["focus".to_string(), "reading".to_string()])
        );
        assert!(snapshot.active_session.is_some());
        assert!(snapshot.checkpoint.is_some());
        assert!(snapshot.sand_state.is_some());
        assert_eq!(snapshot.sand_snapshots.len(), 1);
        assert_eq!(plan.summary().session_count, snapshot.sessions.len() as i64);
        assert_eq!(
