...
for rust_path in Path("src").rglob("*.rs"):
    ...
    updated = updated.replace("sqlite-cli", "sqlite")
    updated = updated.replace('Some("sqlite" | "sqlite" | "sqlite")', 'Some("sqlite")')
    updated = updated.replace('Some("sqlite" | "sqlite")', 'Some("sqlite")')
    if updated != source:
