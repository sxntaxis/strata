...
old = '''pub fn create_backup(path: &Path) -> Result<(), String> {
    if !path.exists() {
        return Ok(());
    }
...
'''
new = '''fn next_publication_nonce() -> u64 {
...
'''
text = replace_once(text, old, new, "publication functions")
if "mod publication_race_tests" in text:
    raise SystemExit("publication race tests already present")
text += r'''
...
print("race-safe legacy publication transform applied")
