...
The former public concept name `drift` is superseded by **idle**. Runtime and storage migration remain implementation work.
...
Layers may carry a user-defined positive, negative, or neutral directional value. This is intended to support balance rather than prescribe morality. Work/leisure is one interpretation, not the only one.
...
- the final semantics of deliberate detach, unexpected termination, inferred elapsed time, and sediment reconstruction;
