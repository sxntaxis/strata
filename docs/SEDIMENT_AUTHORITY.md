...
Current published unit: PLATEAU-001H H4 grain-causal contact + SandState v5 real-profile green. H5 one-shot resize release passed native tests but failed owner visual acceptance; H9 live cadence is native-certified; H10 boundary-release lineage + SandState v6 is the current authored candidate
...
Canvas growth never runs gravity or repacks existing grains. Pending logical mass may occupy newly available capacity through the normal pending-grain placement path. Live viewport widening is also a boundary event: when a visible left or right wall ceases to be a wall and the bottom-connected former-wall surface has newly exposed outward relief `>1`, resize records a **boundary-release lineage** but still moves no grain, consumes no random choice, and changes no mass.

A release lineage is resize-specific confinement physics, not general avalanche activation. It owns the former wall, a fixed outward direction, and the deepest immediately-inward column admitted so far. On normal H4 gravity passes it may perform at most one exact outward diagonal topple. Before admitting another inward column it rechecks the already-admitted face from the former wall inward, so material arriving from deeper columns cannot leave a replacement vertical cliff behind. The next inward column is admitted only after the currently released face is at dynamic repose and that exact adjacent column still has relief `>1` toward the released face. Distinct same-side resize events may coexist while their causal domains are disjoint and normalize only when those domains meet. Hidden release state pauses with the hidden physics basin and resumes on re-expansion.

A grain moved by boundary release does **not** inherit H4 mobility merely from the removed wall. If relief is greater than two, that grain falls vertically under ordinary gravity before another boundary-release topple is allowed. At most one such release grain is in flight; its exact canonical coordinate is persisted separately from H4 mobility. When it joins the bottom-connected face, the release domain is reevaluated. Real support loss created by a boundary topple may still wake exact H4 dependents through the ordinary H4 rule.

H5 one-shot boundary release was native-certified at `3920ab3899f3249569f2dfb8c990e6389cb6fc47` (tree `8f3f69f7baeb15cd60423e5663b36647efd4a68b`), but subsequent owner visual acceptance on the clean H5+H9 candidate showed a former lateral wall remaining as a frozen interior cliff after deliberate expansion. H5 therefore remains useful implementation evidence but is **rejected as sufficient product behavior** and must not be published as the final resize rule. H10 supersedes the one-shot mechanism with the persistent release-lineage semantics above; native certification is pending.

The current viewport is the active live-physics basin. New live grains enter at the visible top edge, gravity and diagonal movement remain within the visible rectangle, and the visible left and right edges act as temporary walls. Grains hidden by shrink remain frozen at their canonical coordinates. When re-expansion makes them visible, ordinary hidden grains reactivate normally and any persisted boundary-release lineage resumes only where its exact causal path is visible; narrowing itself never starts release. Full clear is the one exception to monotonic canvas retention: it removes all placed and pending mass, clears boundary-release transient state, and resets the empty canonical canvas to the current viewport dimensions. Category-specific clearing, including Idle clear, preserves canonical extent.

## SandState persistence

The H10 candidate advances `SandState` to schema version 6. Version 6 retains all H4 v5 state and adds exact resize-release custody: normalized boundary-release domains plus at most one boundary-release grain in flight. Version 5 remains a first-class readable predecessor and upgrades without changing its exact mobilized coordinates or inventing release state. Versions 1 through 4 remain readable; the v4 regional activity field is legacy migration input only.

- Version 1 `pending_grains` vectors migrate deterministically into adjacent runs.
- Version 2 compressed-pending states remain readable and upgrade without invented ingress focus.
- Older JSON with no pending, focus, mobility, or release fields loads those components empty/uninitialized as appropriate for its version.
- Empty pending collections, an uninitialized focus, empty release domains, and no release grain in flight are omitted during serialization.
- The existing persisted RNG state seeds the first ingress focus after a v1/v2 migration; later checkpoints preserve both RNG and focus for deterministic restart continuity.
- `SandState` stores canonical grid dimensions explicitly; recovery through a zero viewport restores them exactly, while an ordinary larger live viewport may monotonically expand the restored canvas. Canonical growth shifts H4 mobility, release-domain coordinates, and any in-flight release grain by the exact canvas offset.
...
- v5/v6 mobility coordinates are canonical row-major, unique, in bounds, and must reference occupied cells; pre-v5 states reject non-empty mobility.
- v6 release domains are canonical, directionally valid, strictly normalized, and in bounds. An in-flight release coordinate must reference a placed grain, must be the outward target of an admitted release-domain column, must not simultaneously carry H4 mobility, and requires at least one live release domain. Pre-v6 states reject release state.
- v6-to-v6 restore preserves exact H4 mobility, release domains, in-flight release custody, RNG, sweep, focus, pending mass, and topology. v5→v6 preserves exact H4 mobility and creates no release state. v1-v4 migration validates and discards regional activity, then seeds only unsupported bottom-connected surface grains once without moving them.

Native v5 validation remains published evidence at `f00b628bd37c42a9b27b2abb4b73b1068c74f551`. PR #89 published H4 on main `67ffd84d3c5c924211ac9a14b52b5749fb07ed8b`; real profile `95446134-3681-4390-84d7-8d900ebbb892` completed v4→v5 and v5→v5 owner restart proof and sqlite-doctor passed. H10's v6 migration/restart boundary is authored but not yet native-certified. SQLite `user_version`, tables, and columns remain unchanged because SandState versioning is inside the serialized sediment payload.
