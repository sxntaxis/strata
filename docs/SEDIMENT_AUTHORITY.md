...
H8 therefore tests a narrower exact-path rupture token under `cfg(test)` only. After a real H4 topple, only an immediately-uphill surface that was already at supported relief exactly `2` before that topple may receive one transient token toward the vacated source side. The token authorizes one exact diagonal topple; it does not copy the grain mobility bit, does not invoke same-column slip-lineage inheritance, does not weaken the static-support rule, and does not persist. Genuine support loss caused by that topple still belongs to ordinary H4. If the topple exposes another immediately-uphill supported relief-2 grain, the token may transfer one column farther; otherwise the front ends. Relief `3` and `>=4`, H6 threshold crossings, already-mobilized grains, radius scans, and unrelated neighbors are excluded. H8 is an A/B experiment, not accepted production semantics.
