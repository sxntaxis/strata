...
| STRATA-D057 | Retroactive history is expressed as an arbitrary past interval assignment: `From < To <= now` plus a target layer. Idle and same-layer overlap are non-conflicting; intersecting different explicit layers require confirmation. The operation is independent of existing session boundaries, never double-counts chronology, and never changes the current selected layer/description even when the active generation must be rebased around corrected past time. | accepted for HISTORY-001D |
